...
export class NFRDataService {
  ...
  async createOrUpdate(
    ...
    const nfrProvisionIds = provisionArray.map(({ provision_id }) => provision_id);
    ...
    if (nfrData) {
      return await this.updateNfrData(nfrDataDto, nfrData, provisionArray, variableArray, nfrProvisions, nfrVariables);
    }
    // else create new nfrData
    const documentTemplate = await this.documentTemplateService.findActiveByDocumentType(nfrDataDto.variant_name);
    ...
    const nfrDataProvisions = provisionArray.map(({ provision_id, free_text }) => {
      const nfrProvision = nfrProvisions.find((provision) => provision.id === provision_id);
    ...
    const nfrDataVariables = variableArray.map(({ variable_id, variable_value }) => {
      const nfrVariable = nfrVariables.find((variable) => variable.id === variable_id);
  ...
  async updateNfrData(
    ...
    for (const provision of provisionArray) {
      const nfrDataProvision = nfrData.nfr_data_provisions.find((p) => p.nfr_provision.id === provision.provision_id);
      const nfrProvision = nfrProvisions.find((p) => p.id === provision.provision_id);

      if (nfrDataProvision && nfrDataProvision.provision_free_text != nfrProvision.free_text) {
        ...
        const newNfrDataProvision: NFRDataProvision = this.nfrDataProvisionRepository.create({
    ...
    for (const variable of variableArray) {
      const nfrDataVariable = nfrData.nfr_data_variables.find((v) => v.nfr_variable.id === variable.variable_id);

      if (nfrDataVariable && nfrDataVariable.data_variable_value != variable.variable_value) {
        ...
        const newNfrDataVariable: NFRDataVariable = this.nfrDataVariableRepository.create({
  ...
  async deleteDataVarsAndProvs(
    ...
    const oldProvisionIds = nfrData
      ? nfrData.nfr_data_provisions.map((dataProvision) => dataProvision.nfr_provision.id)
      : [];
    ...
    const nfrDataProvisionsToDelete = nfrData.nfr_data_provisions
      .filter((dataProvision) => provisionsToDelete.includes(dataProvision.nfr_provision.id))
    ...
    const oldVariableIds = nfrData
      ? nfrData.nfr_data_variables.map((dataVariable) => dataVariable.nfr_variable.id)
      : [];
    ...
    const nfrDataVariablesToDelete = nfrData.nfr_data_variables
      .filter((dataVariable) => variablesToDelete.includes(dataVariable.nfr_variable.id))
  ...
  async findByNfrDataId(nfrDataId: number): Promise<{
    ...
    try {
      ...
      const provisionIds = existingDataProvisions.map((dataProvision) => dataProvision.nfr_provision.id);
      const existingDataVariables = nfrData.nfr_data_variables;
      const variableIds = existingDataVariables.map((dataVariable) => dataVariable.nfr_variable.id);
  ...
  async getVariablesByVariantAndDtid(variantName: string, dtid: number) {
    ...
    const variables: NFRProvisionVariable[] = await this.nfrProvisionService.getVariablesByVariant(
  ...
  async getProvisionsByVariantAndDtid(variantName: string, dtid: number) {
    ...
    // all provisions
    const provisions: NFRProvision[] = await this.nfrProvisionService.getAllProvisions();
    const provisionIds = existingDataProvisions.map((dataProvision) => dataProvision.nfr_provision.id);
  ...
  async makeActive(dtid: number, variant_name: string): Promise<any> {
    ...
    await this.nfrDataRepository.update({ dtid: dtid, variant_name: variant_name }, { active: true });
  }
}
