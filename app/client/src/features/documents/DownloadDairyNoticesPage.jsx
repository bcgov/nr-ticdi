...
export default function DownloadDairyNoticesPage() {
  const job = useSelector(selectDairyNoticesJob);
  ...
  useEffect(() => {
    if (job.id) {
      dispatch(fetchDairyNoticeJob());

      if (pendingDocuments?.length > 0) {
        dispatch(generateDairyNotice(pendingDocuments[0].documentId));
      } else {
        dispatch(completeDairyNoticeJob(job.id));
      }
    }
  ...
  return (
    <section>
      <PageHeading>Download Dairy Notices</PageHeading>
